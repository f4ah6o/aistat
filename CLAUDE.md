...
- `aistat usage` caches per-account Claude usage for 30 seconds (`$CACHE/aistat/usage/claude-v1.json`, keyed by `account.uuid`); `aistat switch`'s decision paths (`FetchUsage`, `FetchForSwitch`) bypass the cache unconditionally. `aistat usage --refresh` bypasses the read path but still writes through.
