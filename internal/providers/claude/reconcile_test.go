...
// TestReconcile_ByteMatch_SetsActive covers D1 branch 1: the live access token
// byte-matches slot[0]; the slot becomes active and is upserted; no profile
// call is made.
...
// TestReconcile_ByteMatch_StaleRefreshToken covers D1 branch 1 with drift: the
// live blob carries a newer refreshToken and expiresAt. After upsert the stored
// slot must reflect the live values.
...
// TestReconcile_ProfileUUID_MatchesSlot1 covers D1 branch 2 with UUID match on
// slot[1] and identity drift: the profile returns a different email for the
// existing UUID; that email is overwritten (D9).
func TestReconcile_ProfileUUID_MatchesSlot1(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
...
// TestReconcile_ProfileUUID_NewSlot covers D1 branch 2 with no UUID match:
// a new slot is inserted.
func TestReconcile_ProfileUUID_NewSlot(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
...
// TestReconcile_Profile401_Fallback covers D1 branch 4 triggered by a 401.
// CaptureWarn must match the generic template; LiveUnstored must be the live blob.
func TestReconcile_Profile401_Fallback(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
...
// TestReconcile_Profile503_Fallback covers D1 branch 4 triggered by a transient
// 503 error.
func TestReconcile_Profile503_Fallback(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
	live := makeCred("tok-c", "ref-c", 3000)
...
// TestReconcile_ProfileMissingFields_StricterDiagnostic covers D1 branch 4
// triggered by ErrProfileMissingFields (HTTP 200 but no uuid/email). CaptureWarn
// must use the stricter diagnostic, not the generic template.
func TestReconcile_ProfileMissingFields_StricterDiagnostic(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
	live := makeCred("tok-c", "ref-c", 3000)
...
// TestReconcile_LiveAbsent_StoredN covers D1 branch 3: no live credential.
// No slot becomes active; stored accounts are returned unchanged; no profile call.
...
// TestReconcile_LiveAbsent_StoredEmpty covers D1 branch 3 with no stored accounts.
...
// ── pathological: duplicate access token ────────────────────────────────────
...
// TestResolveActiveUUID_ByteMatch covers the byte-match fast path: returns the
// slot UUID without calling LookupProfile.
...
// TestResolveActiveUUID_ProfileLookupSuccess covers the profile-lookup path:
// returns the account UUID from the profile (which may or may not match a slot).
func TestResolveActiveUUID_ProfileLookupSuccess(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
	live := makeCred("tok-c", "ref-c", 3000) // no byte-match
...
// TestResolveActiveUUID_Profile401_ReturnsEmpty verifies that a 401 from the
// profile endpoint returns ("", nil) — not an error — so the caller can proceed
// without surfacing a spurious failure.
func TestResolveActiveUUID_Profile401_ReturnsEmpty(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
	live := makeCred("tok-c", "ref-c", 3000)
...
// TestResolveActiveUUID_ProfileTransient_ReturnsErr verifies that a transient
// 503 from the profile endpoint is surfaced as an error so the caller can
// distinguish it from a definitive "no active account" result.
func TestResolveActiveUUID_ProfileTransient_ReturnsErr(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
	live := makeCred("tok-c", "ref-c", 3000)
...
// TestResolveActiveUUID_NoMutation verifies the D11 no-write guarantee:
// ResolveActiveUUID must not mutate any element in in.Stored, even when a
// profile call is made and succeeds.
...
// TestResolveActiveUUID_ProfileMissingFields_ReturnsEmpty verifies that
// ErrProfileMissingFields is treated like 401/403: returns ("", nil) rather
// than surfacing an error, since the caller cannot take meaningful action.
func TestResolveActiveUUID_ProfileMissingFields_ReturnsEmpty(t *testing.T) {
	stored := []accounts.Account{
		makeAccount("uuid-0", "user0@example.com", "tok-a", "ref-a", 1000),
	}
	live := makeCred("tok-c", "ref-c", 3000)
...
// TestReconcile_ProfileNewUUID_EmptyStore verifies that the insert branch of D1
// step 2 works correctly when the stored set is completely empty.
func TestReconcile_ProfileNewUUID_EmptyStore(t *testing.T) {
	live := makeCred("tok-new", "ref-new", 5000)
